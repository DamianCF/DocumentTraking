...
function DepartamentosXcaso() {
    ...
    const [casosDEP, setCasosDEP] = useState([]);

    
    useEffect(() => {

        axios.post('/api/usuario/obtenerdatadepartamento', { idCaso: params.idCaso }).then(res => {
            const datadepartamento = res.data[0]
            setFechaDeInicio(datadepartamento.nombre)
            setFechaDeFinal(datadepartamento.descripcion)
            setCorreo(datadepartamento.correo)
            setTelefono(datadepartamento.telefono)
            setCasosDEP(datadepartamento.tramites)
        ...
        <div>
            ...
            <ul className="nav nav-tabs">
                <li className="nav-item">
                    <a className="nav-link" href={`/editardepartamento/${idCASO}`}>Informacion Departamento</a>
                </li>
                <li className="nav-item">
                    <a className="nav-link active" aria-current="page" href="#">Tramites</a>
                </li>
                <li className="nav-item">
                    <a className="nav-link" href={`/editarempleadosdepartamento/${idCASO}`}>Empleados</a>
                </li>
            </ul>
...
export default DepartamentosXcaso
*/}